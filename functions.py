...
def read_strings_google_api(img_path, draft_list, margin_ratio):
    ...
    for page in response.full_text_annotation.pages:
        ...
        for block in page.blocks:
            prev_pos = -100
            current_draft = ''
            # paragraphは段落または行ごとに切れていることが多い（tweet単位にはなっていない）
            for paragraph in block.paragraphs:
                bbox = paragraph.words[0].bounding_box.vertices
                upper, lower = min([b.y for b in bbox]), max([b.y for b in bbox])
                line_height = lower - upper
                # 直前に認識した文字列の下端（prev_pos）と、今回の文字列の上端がmargin_ratio * line_heightより
                # 大きかった場合、別ツイートと見做す
                # append_stringを実行し、current_draftを初期化
